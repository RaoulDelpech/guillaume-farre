...
export function handleSummary(data) {
  return {
    'tests/load/reports/load-summary.json': JSON.stringify(data, null, 2),
...
function textSummary(data) {
  ...
  trends.forEach((t) => {
    ...
    if (m) {
      lines.push(`${t}: p50=${(m.values['p(50)'] || 0).toFixed(0)}ms p95=${(m.values['p(95)'] || 0).toFixed(0)}ms p99=${(m.values['p(99)'] || 0).toFixed(0)}ms avg=${(m.values.avg || 0).toFixed(0)}ms`);
    }
  });
  return lines.join('\n') + '\n';
}
